...
SNAKE_BLOCK = 20
SNAKE_SPEED = 8
FAST_SPEED = 20  # Added fast speed
...
# Function to read the last level from file
def read_last_level():
    try:
        with open('last_level.txt', 'r') as file:
            last_level = int(file.read())
    except FileNotFoundError:
        last_level = 1
    return last_level

# Function to write the last level to file
def write_last_level(level):
    with open('last_level.txt', 'w') as file:
...
def draw_rocks(rocks):
    for rock in rocks:
        pygame.draw.rect(DIS, BLACK, [rock[0], rock[1], SNAKE_BLOCK, SNAKE_BLOCK])
...
def game_over_msg(score, high_score):
    ...
    reset_text = font.render("Press 'R' to reset high score and level", True, RED)  # Updated text
    DIS.blit(reset_text, [DIS_WIDTH / 2 - reset_text.get_width() / 2, DIS_HEIGHT / 2 + 100])
    pygame.display.update()
    time.sleep(2)

def level_up_msg(level):
    font = pygame.font.SysFont(None, 50)
    level_up_text = font.render(f"Congratulations! You passed level {level - 1}.", True, GREEN)
    DIS.blit(level_up_text, [DIS_WIDTH / 2 - level_up_text.get_width() / 2, DIS_HEIGHT / 2 - level_up_text.get_height() / 2])
    next_level_text = font.render(f"Level {level} starts in a few seconds.", True, GREEN)
    DIS.blit(next_level_text, [DIS_WIDTH / 2 - next_level_text.get_width() / 2, DIS_HEIGHT / 2 + 50])
    pygame.display.update()
    time.sleep(3)
...
def generate_food_position(rocks):
    while True:
        foodx = round(random.randrange(0, DIS_WIDTH - SNAKE_BLOCK) / 20.0) * 20.0
        foody = round(random.randrange(50, DIS_HEIGHT - SNAKE_BLOCK) / 20.0) * 20.0  # Start from y = 50 to avoid buttons
        if not (0 <= foodx < len(SNAKE_COLORS) * 100 and foody < 50) and (foodx, foody) not in rocks:  # Ensure it's not in the button area or on a rock
            return foodx, foody

def generate_rocks(level):
    rocks = []
    if level >= 3:
        for _ in range((level - 1) // 2):  # Increase rock count every two levels
            rock_x = round(random.randrange(0, DIS_WIDTH - SNAKE_BLOCK) / 20.0) * 20.0
            rock_y = round(random.randrange(50, DIS_HEIGHT - SNAKE_BLOCK) / 20.0) * 20.0
            rocks.append((rock_x, rock_y))
    return rocks

def gameLoop(snake_color, food_img, level):
    game_over = False
    game_close = False
    ...
    rocks = generate_rocks(level)
    foodx, foody = generate_food_position(rocks)
    ...
    current_speed = SNAKE_SPEED  # Initialize current speed
    ...
    while not game_over:
        ...
        for event in pygame.event.get():
            ...
            if event.type == pygame.KEYDOWN:
                if event.key == pygame.K_LEFT:
                    ...
                    y1_change = SNAKE_BLOCK
                    x1_change = 0
                current_speed = FAST_SPEED  # Increase speed on key down
            if event.type == pygame.KEYUP:
                current_speed = SNAKE_SPEED  # Reset speed on key up
        ...
        # Check if snake hits the walls, the rocks, or the buttons at the top
        if x1 >= DIS_WIDTH or x1 < 0 or y1 >= DIS_HEIGHT or y1 < 50 or (x1, y1) in rocks:
            game_close = True
        x1 += x1_change
        y1 += y1_change
        DIS.fill(BLUE)
        ...
        level_text = pygame.font.SysFont(None, 35).render("Level: " + str(level), True, BLACK)
        DIS.blit(level_text, [0, 95])
        ...
        if x1 == foodx and y1 == foody:
            ...
            if score % 50 == 0:  # Increase level every 50 points (5 foods)
                level += 1
                write_last_level(level)
                level_up_msg(level)
                rocks = generate_rocks(level)
                snake_list = [[DIS_WIDTH / 2, DIS_HEIGHT / 2]]  # Reset snake position
                length_of_snake = 1
                x1, y1 = DIS_WIDTH / 2, DIS_HEIGHT / 2
                x1_change, y1_change = 0, 0

        CLOCK.tick(current_speed)  # Use the current speed
...
def main_menu():
    ...
    gameLoop(snake_color, selected_food_img, last_level)
