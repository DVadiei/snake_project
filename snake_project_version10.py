...
SNAKE_COLORS = [GREEN, RED, PURPLE, YELLOW]
...
pygame.display.set_caption('Snake Game')  # Corrected line
...
def game_over_msg(score):
    ...
    DIS.blit(info_text, [DIS_WIDTH / 4, DIS_HEIGHT / 2 + 50])
...
def draw_menu():
    ...
    DIS.blit(food_text, [DIS_WIDTH / 3, DIS_HEIGHT / 2 + 100])
    
    food_y = DIS_HEIGHT / 2 + 150
    for idx, food_img in enumerate(FOOD_IMGS):
        DIS.blit(food_img, (DIS_WIDTH / 3 + idx * 120, food_y))

    info_text = menu_font.render("Press 'C' to continue or 'Q' to quit", True, BLACK)
    DIS.blit(info_text, [DIS_WIDTH / 4, DIS_HEIGHT / 2 + 200])
...
def gameLoop(snake_color, food_img):
    ...
    while not game_over:
        ...
        if x1 >= DIS_WIDTH or x1 < 0 or y1 >= DIS_HEIGHT or y1 < 50:  # Prevent snake from going under buttons
            game_close = True
        x1 += x1_change
        y1 += y1_change
        DIS.fill(BLUE)
    ...
    while show_menu:
        ...
        for event in pygame.event.get():
            ...
            if event.type == pygame.MOUSEBUTTONDOWN:
                ...
                food_y = DIS_HEIGHT / 2 + 150
                for idx, color in enumerate(SNAKE_COLORS):
                    if DIS_WIDTH / 3 + idx * 120 <= pos[0] <= DIS_WIDTH / 3 + idx * 120 + 100 and color_y <= pos[1] <= color_y + 50:
                        snake_color = color
                for idx, food_img in enumerate(FOOD_IMGS):
                    if DIS_WIDTH / 3 + idx * 120 <= pos[0] <= DIS_WIDTH / 3 + idx * 120 + 100 and food_y <= pos[1] <= food_y + 50:
